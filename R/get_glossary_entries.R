#' Retrieve Glossary Entries with the DeepL API Pro
#'
...
#' }
#' 
get_glossary_entries <- function(glossary_id, auth_key) {
  ...
  response <- 
    httr::GET(
      url = paste0("https://api.deepl.com/v2/glossaries/", glossary_id, "/entries"),
      httr::add_headers("Authorization" = paste("DeepL-Auth-Key", get_key(auth_key))),
      httr::accept("text/tab-separated-values")
      )
